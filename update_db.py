...
def update_doc(document):
    ...
    if isinstance(document, list):
        document_texts = [doc.text for doc in document if hasattr(doc, 'text') and doc.text is not None]
        document = ' '.join(document_texts) 
    ...
    for i in new_chunks_with_ids:
        if i.metadata['ids'] not in existing_ids:
            new_chunks.append(i)
